...
var MaxLength = require('./validators.js').MaxLength;
var Required = require('./validators.js').Required;
var Enumerated = require('./validators.js').Enumerated;
var FutureDate = require('./validators.js').FutureDate;
...
var FormStateMachine = class {
  constructor(fetchUrl, submitPath, postCompile, context, changedEvent, forceCall, offset = 'actionable') {
    ...
    this.offset = offset;
    _.extend(this, Deferrable(FormStateMachine, !!this.noCache, false),
      StateMachineActions());
  }
  reset(){
    this.state = [];
  ...
  getState() {
    ...
    return {
      ...
      error: that.error,
      retryEvent: that.retryEvent,
      retryEventArgs : that.retryEventArgs,
      submitError: that.submitError,
      submitRetryEvent: that.submitRetryEvent,
      submitRetryEventArgs : that.submitRetryEventArgs,
  ...
  init(id, type, eventBus, callback, mode, options = null, failure = null) {
    ...
    var queryParams = type ? { type: type } : {};
  ...
  compile(data) {
    ...
    this.config = Immutable.fromJS(data.config);
    var groupValidators = {};
    var that = this;
    var state = data.definitions.map(function(d, index) {
      var validators = [];
      if (!_.contains(d.validators, 'optional')) {
        validators.push(new Required());
      }
      var boolean_types = ['boolean', 'toggle-boolean'];
      if(d.optional || _.includes(boolean_types, d.type)){
        validators = [];
      }
      if (_.contains(d.validators, 'enum')) {
        validators.push(new Enumerated());
      }
      if (_.contains(d.validators, 'future_date')) {
        validators.push(new FutureDate());
      }
      if (_.contains(d.validators, "group::all_or_none")) {
        validators = [];
        groupValidators[d.tag] = 'allOrNull';
      }
      if (_.contains(d.validators, "max_allowed")) {
        validators.push(new MaxLength( d.max_chars));
      }
