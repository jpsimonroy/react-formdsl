	...
	return {
      ...
      var formConfig = 
			...
			var formBody = allElements.map((e, index) => e.get('type') === 'section' ?
                        ...
                        that._elementFor(e.get('value'), [e.get('value')], config ? config.toJS()  : {}));
      ...
      if(_.isEqual(layout, "2-column")){
        var column1 = <div className="col-md-6">
          {formBody.slice(0, Math.round(formBody.size/2))}
        </div>;
        var column2 = <div className="col-md-6">
          {formBody.slice(Math.round(formBody.size/2) +1)}
        </div>;
        formBody = <div>
          {column1}
          {column2}
